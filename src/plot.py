...
def plot_trend_lines(output_base_name, state, alphabet_size_list, grammar_rules_size_list, text_size_list):
    trend_line_list = []
    legend_list = []
    color_list = []
    ...
    legend_list.extend([r"$\sigma_{d}$", r"$r_{d}$", r"$n_{d}$"])

    size_lists = [alphabet_size_list[:-1], alphabet_size_list[1:], grammar_rules_size_list[1:], text_size_list[:-1], text_size_list[1:]]
    label_list = [r"$\sigma_{d}$", r"$\sigma_{d + 1}$", r"$r_{d + 1}$", r"$n_{d}$", r"$n_{d + 1}$"]
    ...
    (figure_, axes_) = pyplot.subplots(nrows = 1, ncols = 1, figsize = (15, 15))
    ...
    axes_.set_xticks(list(range(int(trend_line_size * 1.4))))
    axes_.ticklabel_format(axis = 'y', scilimits = (0, 0))
    axes_.tick_params(axis = "both", which = "both", labelsize = 18)
    axes_.legend(filtered_legend_list, fontsize = 16)
...
def parse_csv_file (input_root_directory, output_root_directory, file):
    with open(os.path.join(input_root_directory, file), 'r') as input_file:
        ...
        text_size_list = [int(value) for value in lines[2].split(',')]

    output_base_name = os.path.join(output_root_directory, os.path.splitext(file)[0])
    for state in ["all", "warning", "safe"]:
        plot_trend_lines(output_base_name, state, alphabet_size_list, grammar_rules_size_list, text_size_list)
