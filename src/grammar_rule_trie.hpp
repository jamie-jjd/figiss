...
)
{
  ...
  uint8_t prev_sl {S};
  auto rfirst {std::prev(std::end(text), 2)};
  auto rit {rfirst};
  auto rlast {std::prev(std::begin(text))};
  while (rit != rlast)
  {
    ...
    if (is_new_rule)
    {
      size_rules += std::difference(rit, rfirst);
    }
    ++size_gc_text;
  }

  return std::pair(size_rules, size_gc_text);
}
...
struct gc_index
{
  /* int vector for concantenated grammar rules */
  sdsl::int_vector<8> rules;

  /* root of trie for grammar rules in lexicographical order */
  std::shared_ptr<trie_node> lex_root;

  /* root of trie for grammar rules in colexicographical order */
  std::shared_ptr<trie_node> colex_root;

  /*
    bit vector for character range of sorted grammar compressed text
    , where characters are mapped from grammar rule in lexicographical order
  */
  sdsl::sd_vector<> lex_c;

  /*
    wavelet tree on bwt of grammar compressed text
    , where characters are mapped from grammar rule in lexicographical order
  */
  sdsl::wt_int<> lex_f;
  ...
  /*
    wavelet tree on bwt of grammar compressed text
    , where characters are mapped from grammar rule in colexicographical order
  */
  sdsl::wt_int<> colex_f;

  grammar_compressed_index (sdsl::int_vector<8> const &text)
  {
    using size_type = typename std::iterator_traits<sdsl::int_vector<8>>::size_type;

    auto [size_rules, size_gc_text] {insert_rules(text, lex_root, colex_root)};

    rules.resize(size_rules);
    rebuild_lex_rule_trie(text, lex_root);
    rebuild_colex_rule_trie(text, );
